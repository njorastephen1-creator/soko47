...
function BrowsePage() {
  ...
  const hasMore = products && products.length === PAGE_SIZE;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-28 pt-8 md:pb-8">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <button onClick={() => { setPage(1); nav({ q, county }); }} className={chip(!category)}>All</button>
        {CATEGORIES.map((c) => (
          <button key={c.slug} onClick={() => { setPage(1); nav({ q, category: c.slug, county }); }} className={chip(category === c.slug)}>{c.name}</button>
        ))}
      </div>
      {activeCat && (
        <div className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-accent/40 bg-accent/10 p-3">
          <button onClick={() => { setPage(1); nav({ q, category, county }); }} className={subChip(!sub)}>All {activeCat.name}</button>
          {activeCat.subs.map((s) => (
            <button key={s} onClick={() => { setPage(1); nav({ q, category, sub: s, county }); }} className={subChip(sub === s)}>{s}</button>
          ))}
        </div>
      )}
      <select value={county || ""} onChange={(e) => { setPage(1); nav({ q, category, sub, county: e.target.value || undefined }); }} className="mt-4 rounded-md border border-border bg-card px-3 py-2 text-sm">
      ...
      {!isLoading && allProducts.length === 0 && <p className="py-16 text-center text-muted-foreground">No products match yet. Try another word or category.</p>}
    </div>
  );
}
`;
