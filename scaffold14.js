...
'src/routes/_authenticated/help.tsx': `import { createFileRoute } from "@tanstack/react-router";
...
if (!chrome.includes('to="/help"')) {
  chrome = chrome.replace('<DropdownMenuItem asChild><Link to="/orders">My orders</Link></DropdownMenuItem>', '<DropdownMenuItem asChild><Link to="/orders">My orders</Link></DropdownMenuItem>\n                <DropdownMenuItem asChild><Link to="/help">Help & support</Link></DropdownMenuItem>');
  fs.writeFileSync('src/components/site-chrome.tsx', chrome);
  console.log('Patched site-chrome.tsx (Help link)');
}
console.log('DONE: wave 2 modules');