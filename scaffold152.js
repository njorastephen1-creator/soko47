...
function SocialPage() {
  ...
  const tagCounts: any = {};
  (posts || []).forEach((p: any) => tagsOf(p).forEach((t: string) => { tagCounts[t] = (tagCounts[t] || 0) + 1; }));
  ...
  if (sort === "trending") feed.sort((a: any, b: any) => (likesCount[b.id] || 0) - (likesCount[a.id] || 0));
    ...
    try {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        if (["complete", "completed", "paid", "success"].includes(state)) {
          ...
          qc.invalidateQueries(); toast.success("Social active for 7 days - post your first ad!"); setPaying(false); return;
        }
  ...
  const createPost = async () => {
    ...
    if (!title.trim() && !desc.trim() && !url) return toast.error("Add a title, description or media");
    const { error } = await supabase.from("posts").insert({ user_id: session.user.id, vendor_id: vendor ? vendor.id : null, kind, title: title.trim() || null, body: desc.trim() || null, tags: tags.trim() || null, media_url: url || null, author_name: myName, author_shop: vendor ? vendor.shop_name : null });
    if (error) return toast.error(error.message);
    setTitle(""); setDesc(""); setTags(""); setMedia(""); setMediaUrl("");
    qc.invalidateQueries(); toast.success("Your ad is live on Soko47 Social!");
  };
  ...
  const share = async (p: any) => { const link = window.location.origin + "/social#" + p.id; if (navigator.share) navigator.share({ title: p.title || "Soko47 Social", url: link }); else { navigator.clipboard.writeText(link); toast.success("Link copied - share it anywhere"); } };
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-28 pt-8 md:pb-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Business ads that move - videos, photos and stories from traders across Kenya. Free to watch, KSh 100/week to post.</p>
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <Input className="w-44" placeholder="Search ads..." value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      {popularTags.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {popularTags.map((t) => (<button key={t} onClick={() => setActiveTag(activeTag === t ? "" : t)} className={"rounded-full px-2 py-0.5 text-[11px] font-semibold " + (activeTag === t ? "bg-accent text-white" : "bg-accent/15 text-accent-deep")}>#{t}</button>))}
        </div>
      )}
      {!session ? (
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-5">
          <h2 className="font-semibold">Post a business ad{isAdm ? " (admin - free)" : ""}</h2>
          <div className="mt-2 flex gap-2">{["photo", "video", "blog"].map((k) => (<button key={k} onClick={() => setKind(k)} className={"rounded-full px-3 py-1 text-xs font-semibold capitalize " + (kind === k ? "bg-primary text-primary-foreground" : "bg-secondary")}>{k}</button>))}</div>
          <div className="mt-3 space-y-3">
            ...
            <div><Label>Description</Label><Textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={2} placeholder="Tell buyers why they need it..." /></div>
            <div><Label>Tags (comma separated)</Label><Input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="e.g. avocado, fresh, nakuru" /></div>
      ...
      <div className="mt-6 space-y-4">
        {feed.map((p: any) => (
          <div key={p.id} id={p.id} className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold">{p.author_name}{p.author_shop ? " · " + p.author_shop : ""}</p><span className="text-xs text-muted-foreground">{when(p.created_at)}</span></div>
        ...
        {feed.length === 0 && <p className="text-sm text-muted-foreground">No ads match - be the first to post!</p>}
      </div>
    </div>
  );
}
