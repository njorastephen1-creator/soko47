...
function SocialPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-28 pt-8 md:pb-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Business ads that move - free to watch, KSh 100/week to post.</p>
      ...
      {!session ? (<div className="mt-6 rounded-2xl border border-border bg-card p-5 text-sm">Sign in to like, comment, save, follow and post.</div>) : !socialActive ? (
        <div className="mt-6 rounded-2xl border border-accent/40 bg-accent/10 p-5">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2"><Input className="w-44" placeholder="M-Pesa phone" value={payPhone} onChange={(e) => setPayPhone(e.target.value)} /><Button onClick={paySocial} disabled={paying}>{paying ? "Waiting..." : "Activate - KSh 100/week"}</Button></div>
        </div>
      ) : (
        ...
        {feed.map((p: any) => (
          <div key={p.id} id={p.id} className={"rounded-2xl border bg-card p-4 " + (boosted(p) ? "border-accent ring-1 ring-accent" : "border-border")}>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
              ...
              {session && p.user_id === session.user.id && !boosted(p) ? (<button onClick={() => boost(p)} className="flex items-center gap-1 rounded-full bg-accent/15 px-2 py-0.5 text-[11px] font-bold text-accent-deep"><Zap className="size-3" /> Boost KSh 50</button>) : null}
            </div>
            {openComments === p.id ? (
