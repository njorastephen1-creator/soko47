import fs from 'fs';
fs.writeFileSync('src/routes/_authenticated/settings.tsx', `import { createFileRoute } from "@tanstack/react-router";
...
if (!chrome.includes('to="/settings"')) {
  chrome = chrome.split('<DropdownMenuItem asChild><Link to="/orders">My orders</Link></DropdownMenuItem>').join('<DropdownMenuItem asChild><Link to="/orders">My orders</Link></DropdownMenuItem>\n                <DropdownMenuItem asChild><Link to="/settings">Settings</Link></DropdownMenuItem>');
  fs.writeFileSync('src/components/site-chrome.tsx', chrome);
  console.log('Patched header dropdown (Settings link)');
}
