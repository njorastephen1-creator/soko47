...
`);
console.log('Created Payment Hub');
let checkout = fs.readFileSync('src/routes/checkout.tsx', 'utf8');
if (checkout.includes('navigate({ to: "/orders" });')) {
  checkout = checkout.split('navigate({ to: "/orders" });').join('navigate({ to: "/pay/$id", params: { id: order.id } });');
  fs.writeFileSync('src/routes/checkout.tsx', checkout);
  console.log('Checkout now lands on Payment Hub');
}
console.log('DONE');