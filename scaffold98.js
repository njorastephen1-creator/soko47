...
if (!c.includes('select-none rounded-xl')) {
  c = c.split('className={"rounded-xl px-3 py-2 text-sm shadow-sm " + bubbleBg + " " + ring}>').join('className={"select-none rounded-xl px-3 py-2 text-sm shadow-sm " + bubbleBg + " " + ring} style={{ WebkitTouchCallout: "none", userSelect: "none" } as any}>');
  n++;
}
...
console.log('DONE:', n, 'patches - native selection disabled on bubbles');