...
// Create a simple og.js endpoint (no brackets in filename)
...
`);
console.log('api/og.js created (no brackets)');

// Delete the bracket version
try { fs.unlinkSync('api/og/[id].js'); console.log('removed api/og/[id].js'); } catch {}

// Update vercel.json to route /product/:id to /api/og?id=:id
...
`);
console.log('vercel.json updated to use query param');