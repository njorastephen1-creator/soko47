...
function SocialPage() {
  ...
  const toggleLike = async (id: string) => { if (!session) return toast.error("Sign in to like"); const post = (posts || []).find((x: any) => x.id === id); if (myLikes[id]) { await supabase.from("post_likes").delete().eq("post_id", id).eq("user_id", session.user.id); } else { await supabase.from("post_likes").insert({ post_id: id, user_id: session.user.id }); if (post) notify(post.user_id, myName + " liked your ad", post.title || "Your ad"); } qc.invalidateQueries({ queryKey: ["social-likes"] }); };
  const toggleSave = async (id: string) => { if (!session) return toast.error("Sign in to save"); if (savedMap[id]) await supabase.from("post_saves").delete().eq("post_id", id).eq("user_id", session.user.id); else await supabase.from("post_saves").insert({ post_id: id, user_id: session.user.id }); qc.invalidateQueries({ queryKey: ["social-saves"] }); };
  ...
  const recordView = async (id: string) => { if (!session) return; const { data } = await supabase.from("post_views").select("post_id").eq("post_id", id).eq("user_id", session.user.id).maybeSingle(); if (!data) { await supabase.from("post_views").insert({ post_id: id, user_id: session.user.id }); qc.invalidateQueries({ queryKey: ["social-views"] }); } };
    ...
    <div className="mx-auto max-w-2xl px-4 pb-28 pt-8 md:pb-8">
      ...
      {!session ? (<div className="mt-6 rounded-2xl border border-border bg-card p-5 text-sm">Sign in to like, comment, save, follow and post.</div>) : !socialActive ? (
        <div className="mt-6 rounded-2xl border border-accent/40 bg-accent/10 p-5">
          <h2 className="font-semibold">Unlock posting - KSh {Number(settings.social_price)}/week</h2>
      ...
      <div className="mt-6 space-y-4">
        {feed.map((p: any) => (
          <div key={p.id} id={p.id} className={"rounded-2xl border bg-card p-4 " + (boosted(p) ? "border-accent ring-1 ring-accent" : "border-border")}>
            ...
            {p.kind !== "video" && p.media_url ? (<img src={p.media_url} alt={p.title || "ad"} className="mx-auto mt-3 block max-w-full rounded-xl" style={{ maxHeight: "70vh" }} />) : null}
            {p.kind === "video" && p.media_url ? (<video src={p.media_url} controls autoPlay muted loop playsInline className="mx-auto mt-3 block max-w-full rounded-xl" style={{ maxHeight: "65vh" }} />) : null}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-4 text-base">
              ...
              <button onClick={() => { setOpenComments(openComments === p.id ? null : p.id); recordView(p.id); }} className="flex items-center gap-1 text-muted-foreground"><MessageCircle className="size-6" /> {(commentsByPost[p.id] || []).length}</button>
              <span className="flex items-center gap-1 text-muted-foreground"><Eye className="size-6" /> {(p.views || 0) + (viewsCount[p.id] || 0)}</span>
              <button onClick={() => toggleSave(p.id)} className={"flex items-center gap-1 " + (savedMap[p.id] ? "text-accent-deep" : "text-muted-foreground")}><Bookmark className={"size-6 " + (savedMap[p.id] ? "fill-accent" : "")} /></button>
